...
function domainIdentity(value) {
  ...
  const supportedSuffix = parsed.isIcann || parsed.isPrivate || parsed.publicSuffix === 'test';
  if (!supportedSuffix || parsed.isIp || !parsed.hostname || !parsed.domain) return null;
  return { hostname: parsed.hostname, registrableDomain: parsed.domain };
}
