...
function createMatrixThreadDelivery({db,transport,previewService,clock=()=>new Date(),fromAddress,fromHeader,replyToAddress=fromAddress,messageIdDomain,requireCanonicalCutover=process.env.NODE_ENV==='production'}={}){
 ...
 const mailFrom=String(fromHeader||from).trim(),headerAddress=(mailFrom.match(/<([^<>]+)>\s*$/)||[])[1]||mailFrom;
 if(/[\r\n\0]/.test(mailFrom)||headerAddress.toLowerCase()!==from)throw new Error('matrix thread sender header mismatch');
