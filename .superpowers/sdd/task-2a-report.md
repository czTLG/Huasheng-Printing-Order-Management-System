...
- Clause claims RED: `We would like to discuss guaranteed barrier performance of our high-barrier valve pouch` passed without performance evidence. GREEN: sensitive detection now splits sentences into claim clauses. It removes only an evidence-backed product descriptor from the semantic scan of its discussion clause; the complete clause remains the exact evidence key. Guaranteed/certified/price/delivery/lead-time/performance assertions in that clause or sibling clauses remain hard-gated.
- Strict questions RED: the broad question-prefix fallback released `Could you confirm our closure is magnetic? / 请确认我们的封口为磁吸式？`. GREEN: every fact gate now reuses the single Task 2 assertion-aware classifier. Interrogative `是否`/`还是` and `请问` forms are recognized without masking `our/我们的 + is/为/是/已/拥有` assertion structures. Both direct and confirm-wrapped unknown properties fail closed.
- Option normalization RED: the exact PET/zipper question scored 75 because proposed options entered asserted facts and English/Chinese option extraction differed. GREEN: asserted fact alignment operates on raw bodies after strict request removal; question options have separate role/value intents. PET no longer also matches PE in Chinese, and `use/使用 ... zipper/拉链` normalizes symmetrically. Both official option fixtures assert score 100, all component gates, empty hard failures, and final acceptance.
- Typed evidence RED: evidence `size 100mm` authorized `thickness 100mm`, the reverse also passed, and bare `100mm` populated both roles. GREEN: explicit size and thickness retain separate roles; bare length evidence is conservatively untyped and cannot support either explicit role. Equivalent canonical fixtures now provide typed size/thickness evidence, while weight remains intrinsically typed by unit.
- Parser integrity: question and assertion analysis now consumes raw bodies so paragraph newlines cannot be collapsed into `Best regards Could you ...`, which previously changed the classifier boundary.
- R6 Critical follow-up RED: broad Chinese `阻隔...袋` removal swallowed embedded `FDA认证`, and finite copula lists treated field-style `closure: magnetic` plus `uses/采用 magnetic technology` confirmations as safe. GREEN: product intent removes only exact evidence-backed descriptor tokens (`high-barrier/高阻隔`, `valve pouch/带阀袋`) while ten bilingual embedded-sensitive variants retain their independent exact-evidence gates. Confirmation structure now treats colons and sender-owned `our/我们的` properties as assertions independent of the predicate; genuine `whether/是否/偏好...还是...` option questions remain safe. Every negative and positive fixture asserts score, component maxima, final pass state, and exact hard failures.
