...
async function assertHealthy({
  ...
  if (!isAlive(state.inboxWatcherPid)) throw new Error('inbox watcher is not running');
...
async function run() {
  ...
  const inboxWatcher = spawn(process.execPath, ['/workspace/scripts/matrix-inbox-watch.js'], { stdio: 'inherit', env: process.env });
  const bridge = spawn('feishu-codex-bridge', ['run', '--bot', 'stream-node'], { stdio: 'inherit', env: process.env });
  const children = [watcher, orderWatcher, inboxWatcher, bridge];
  writeState(statePath, { watcherPid: watcher.pid, orderWatcherPid: orderWatcher.pid, inboxWatcherPid: inboxWatcher.pid, bridgePid: bridge.pid, startedAt: new Date().toISOString() });
  ...
  inboxWatcher.once('exit', (code, signal) => stop(code && code > 0 ? code : 1, `inbox watcher exited (${code ?? signal ?? 'unknown'})`));
  ...
  inboxWatcher.once('error', error => stop(1, `inbox watcher failed: ${error.message}`));
