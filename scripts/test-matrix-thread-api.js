...
for(const value of ['/thread-routes/prepare','/thread-routes/:id/approve','/thread-routes/:id/preview','/thread-routes/:id/send'])assert.ok(route.includes(value),`missing ${value}`);
for(const value of ['prepareThreadRoute','approveThreadRoute','previewThreadRoute','confirmThreadRoute'])assert.ok(client.includes(value),`missing ${value}`);
