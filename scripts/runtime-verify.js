...
function main() {
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(bundle, 'manifest.json'))); const db = new Database(path.join(bundle, 'database', 'app.db'), { readonly: true, fileMustExist: true });
  let result;
